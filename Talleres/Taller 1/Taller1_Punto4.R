...
#Verificar tipo de convergencia de cos(1/x) en x = 1

Gx <- expression(cos(1/x))

dx = D(Gx,"x")
gdx <- function(x) eval(dx)


Gx <- function(x) cos(1/x)
raiz = newtonRaphson(Gx,x0 = 1,maxiter =1000,tol = 1e-8 )
if(abs(gdx(raiz[[1]])< 1))
{
 cat("Converge")    
}else
...
t1<-seq(0,2,0.01)
plot(t1,Ft(t1),type="l",col="red", ylim =c(-5,3), xlab = "x", ylab="y")
par(new=TRUE)
plot(t1,Gt(t1),type="l",col="blue", ylim= c(-5,3),xlab = "x", ylab="y")
