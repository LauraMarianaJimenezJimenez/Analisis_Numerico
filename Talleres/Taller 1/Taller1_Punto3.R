...
#Encontrar el valor teÃ³rico
auxiliar <- uniroot(h, c(-1.8,1), tol = 1e-8) 
#De acuerdo con esta funciÃ³n, el valor teorico serÃ­a teo
teo <- auxiliar$root
...
Newton = function(Fx, Gx, x0, tol, maxiter)
{
  ...
  plot.function(Fx, xlim=c(0,0.5), ylim=c(0,1),  main = "RelaciÃ³n error Newton generalizado",xlab = " Error i ",ylab = " Error i+1 ",col ="white")
  
  #imprime la relaciÃ³n de error -> Convergencia lineal
  ...
  {
    cat("Numero iteraciones = ", n, " RaÃ­z = ", x1, " f(x) = ", Fx(x1), " Error estimado = ", error, "\n") 
  }
...
plot.function(Fx, xlim=c(0,0.3), ylim=c(0,1),  main = "RelaciÃ³n error Newton Raphson",xlab = " Error i ",ylab = " Error i+1 ",col ="white")
expresion <- expression (exp(1)^x - x - 1 )
newtonRaphson(1,0,1e-8,29,Fx,Gx,expresion)

plot(Fx, xlim = c(-20,20), ylim=c(-20,20), col = "purple", main = "GrÃ¡fica de e^x - x - 1")
